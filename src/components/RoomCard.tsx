...
export default function RoomCard({
  ...
  const whatsappHref = buildRoomBooking({
    activity: 'Consulta sobre alquiler',
  })
    ...
    <div className="border-border bg-card overflow-hidden rounded-lg border transition-all duration-300 hover:shadow-md">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="rounded-full bg-linear-to-r from-violet-600 to-pink-600 px-3 py-1 text-xs font-medium text-white">
            {room.type === 'salon' ? 'Salón' : 'Patio'}
          </span>
        </div>
        {room.capacity && (
        ...
        {room.features && room.features.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {room.features
                .slice(0, showFullDetails ? room.features.length : 3)
                .map((feature, index) => (
                  <span
                    key={index}
